...
import { FiTrendingUp, FiTrendingDown, FiDollarSign, FiShoppingCart, FiUsers, FiPercent } from "react-icons/fi";
      ...
      <div className="space-y-6 p-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-600">
              分析期間：{new Date(reportData.summary.period.startDate).toLocaleDateString()} - {new Date(reportData.summary.period.endDate).toLocaleDateString()}
            </p>
          </div>
          ...
          <div className="flex space-x-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">時間週期</label>
              <select
                value={period}
                  ...
                  <dl>
                    ...
                    <dd className="flex items-baseline">
                      ...
                      <div className={`ml-2 flex items-baseline text-sm font-semibold ${
                        reportData.summary.orderGrowth >= 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {reportData.summary.orderGrowth >= 0 ? <FiTrendingUp className="h-4 w-4" /> : <FiTrendingDown className="h-4 w-4" />}
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    ...
                    <dd className="flex items-baseline">
                      ...
                      <div className={`ml-2 flex items-baseline text-sm font-semibold ${
                        reportData.summary.revenueGrowth >= 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
                        {reportData.summary.revenueGrowth >= 0 ? <FiTrendingUp className="h-4 w-4" /> : <FiTrendingDown className="h-4 w-4" />}
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-5">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">平均訂單金額</dt>
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-5">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">客戶回購率</dt>
                    <dd className="text-2xl font-semibold text-gray-900">
          ...
          <div className="bg-white overflow-hidden shadow rounded-lg">
            <div className="p-5">
              <div className="flex items-center">
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className="text-sm font-medium text-gray-500 truncate">平均客戶訂單</dt>
                    <dd className="text-2xl font-semibold text-gray-900">
                      {reportData.customerAnalysis.averageOrdersPerCustomer.toFixed(1)}
                    </dd>
                  </dl>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white shadow rounded-lg p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">訂單狀態分布</h3>
            <div className="space-y-3">
              {Object.entries(reportData.ordersByStatus).map(([status, count]) => (
                <div key={status} className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-gray-900 font-medium">{count} 筆</div>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Top Products */}
          <div className="bg-white shadow rounded-lg p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">熱銷商品 Top 5</h3>
            <div className="space-y-3">
              {reportData.topProducts.slice(0, 5).map((product, index) => (
                <div key={product.id} className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-xs text-gray-500">銷量: {product.quantity}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white shadow rounded-lg p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">商品類別表現</h3>
            <div className="space-y-3">
              {reportData.categoryStats.map((category) => (
                <div key={category.category} className="flex items-center justify-between">
                  <div className="text-sm text-gray-900">{getCategoryLabel(category.category)}</div>
                  <div className="text-right">
                    <div className="text-sm font-medium text-gray-900">{formatCurrency(category.revenue)}</div>
                    <div className="text-xs text-gray-500">{category.orders} 筆訂單</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Top Customers */}
          <div className="bg-white shadow rounded-lg p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-4">VIP 客戶 Top 5</h3>
            <div className="space-y-3">
              {reportData.customerAnalysis.topCustomers.slice(0, 5).map((customer, index) => (
                <div key={customer.email} className="flex items-center justify-between">
                  <div className="flex items-center">
                    <span className="text-sm text-gray-500 w-6">#{index + 1}</span>
                    <span className="text-sm text-gray-900 ml-2">{customer.email.replace(/(.{3}).*(@.*)/, '$1***$2')}</span>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-medium text-gray-900">{formatCurrency(customer.revenue)}</div>
                    <div className="text-xs text-gray-500">{customer.orders} 筆訂單</div>
                  </div>
                </div>
              ))}
            </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {reportData.revenueByPeriod.map((period) => (
                  <tr key={period.period}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {formatCurrency(period.orders > 0 ? period.revenue / period.orders : 0)}
                    </td>
                  </tr>
                ))}
              </tbody>
