...
export async function GET(request: Request) {
  try {
    ...
    const userBehaviorAnalysis = calculateUserBehavior(ordersInPeriod, allUsers);
    const conversionAnalysis = calculateConversionMetrics(ordersInPeriod, usersInPeriod);
...
function calculateUserBehavior(orders: any[], users: any[]) {
  ...
  const orderFrequencies = Array.from(userOrderMap.values()).map(userOrders => userOrders.length);
  
  return {
    averageOrdersPerUser: orderFrequencies.length > 0 
...
function calculateConversionMetrics(orders: any[], newUsers: any[]) {
  const usersWhoOrdered = new Set(orders.map(order => order.shippingInfo.email));
  const conversionRate = newUsers.length > 0 
    ? (usersWhoOrdered.size / newUsers.length) * 100 
    : 0;
...
function calculateProductAnalytics(orders: any[]) {
  ...
  orders.forEach(order => {
    order.items.forEach((item: any) => {
      // Track purchases
      productPurchases.set(item.id, (productPurchases.get(item.id) || 0) + item.quantity);
      productRevenue.set(item.id, (productRevenue.get(item.id) || 0) + (item.price * item.quantity));
      
      // Mock view data (would come from GA)
      productViews.set(item.id, (productViews.get(item.id) || 0) + item.quantity * 10);
    });
  });

  const productAnalytics = Array.from(productPurchases.entries()).map(([productId, purchases]) => {
...
function calculateTimeBasedAnalytics(orders: any[]) {
  ...
  return {
    peakHours: hourlyData.map((count, hour) => ({ hour, orders: count }))
...
function analyzePurchasePatterns(userOrderMap: Map<string, any[]>) {
  ...
  userOrderMap.forEach(userOrders => {
    ...
    for (let i = 1; i < userOrders.length; i++) {
      const daysBetween = (new Date(userOrders[i].createdAt).getTime() - 
                          new Date(userOrders[i-1].createdAt).getTime()) / (1000 * 60 * 60 * 24);
    ...
    userOrders.forEach(order => {
      ...
      patterns.seasonalTrends.set(month, (patterns.seasonalTrends.get(month) || 0) + 1);
      
      order.items.forEach((item: any) => {
        patterns.categoryPreferences.set(item.category, 
          (patterns.categoryPreferences.get(item.category) || 0) + 1);
      });
    });
  });

  patterns.averageDaysBetweenOrders = intervalCount > 0 ? totalDaysBetween / intervalCount : 0;

  return {
    averageDaysBetweenOrders: patterns.averageDaysBetweenOrders,
    seasonalTrends: Array.from(patterns.seasonalTrends.entries()).map(([month, count]) => ({
