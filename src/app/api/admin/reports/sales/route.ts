...
export async function GET(request: Request) {
  try {
    ...
    orders.forEach((order) => {
      ...
      customerRevenue.set(
        email,
        (customerRevenue.get(email) || 0) + order.total
      );
    });

    const repeatCustomers = Array.from(customerOrders.entries()).filter(
      ([_, count]) => count > 1
    ).length;
    const customerRetentionRate =
      totalOrders > 0 ? (repeatCustomers / customerOrders.size) * 100 : 0;
    ...
    return NextResponse.json({
      ...
      customerAnalysis: {
        ...
        averageOrdersPerCustomer:
          customerOrders.size > 0 ? totalOrders / customerOrders.size : 0,
        topCustomers: Array.from(customerRevenue.entries())
          .map(([email, revenue]) => ({
            email,
            revenue,
            orders: customerOrders.get(email) || 0,
          }))
          .sort((a, b) => b.revenue - a.revenue)
          .slice(0, 10),
      },
